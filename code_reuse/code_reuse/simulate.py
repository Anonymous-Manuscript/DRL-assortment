...
for i in range(M):
    ...
    '''
    if N>10:
        LATENT_CLASS_SETTINGS['linear_solver_partial_time_limit'] = 60
        LATENT_CLASS_SETTINGS['non_linear_solver_partial_time_limit'] = 60
        LATENT_CLASS_SETTINGS['solver_total_time_limit'] = 300'''
    ...
    #Settings = set_Settings(RANKED_LIST_SETTINGS)
    #rl_estimator = RankedListExpectationMaximizationEstimator().with_this(MIPMarketExplorer())
    #rl = rl_estimator.estimate_with_market_discovery(rl, train_data)
    #exp.estimate_from_transaction(products, train_data)
    # 4
    start_time = time.time()
    '''
    if N>10:
        MC_SETTINGS['linear_solver_partial_time_limit'] = 60
        MC_SETTINGS['non_linear_solver_partial_time_limit'] = 60
        MC_SETTINGS['solver_total_time_limit'] = 300'''
...
'''
# examine whether the saved models can be recovered
i = 0
choice_models = []
for choice_model, cm_name in zip([MultinomiallogitModel, LatentClassModel, ExponomialModel, MarkovChainModel],['mnl', 'lcmnl', 'exp', 'mc']):
    features = choice_model.feature()
    data = {'products':products}
    for feature in features:
        file_name = 'GT/'+args.folder+'/Cus_type'+str(i)+'/'+cm_name+'_'+feature+'.npy'
        data[feature] = list(np.load(file_name))
    choice_models.append(choice_model.from_data(data))
mnl = choice_models[0]
print(mnl.etas)
assort_net = torch.load('GT/'+args.folder+'/Cus_type'+str(i)+'/AssortNet'+str(width)+'.pth')
print(assort_net.products)
'''
