...
if args.DQN:
    args.folder = args.GT+'N='+str(args.num_products)+'M='+str(args.num_cus_types)+\
              'k='+str(args.k)+'L='+str(args.L)+'T='+str(args.T)+'INV='+str(args.ini_inv)+'DQN'+args.CM+args.reuse_type
else:
    args.folder = args.GT+'N='+str(args.num_products)+'M='+str(args.num_cus_types)+\
              'k='+str(args.k)+'L='+str(args.L)+'T='+str(args.T)+'INV='+str(args.ini_inv)+network+args.CM+args.reuse_type
...
if args.UB:
    stream = os.path.join('GT', GT_folder+'/UB_INV'+str(args.ini_inv)+'C'+str(args.cardinality)+str(args.reuse_type))
    logger = setup_logger(name='Train', level=20, stream = stream)
    args.logger = logger
    #logger.info(args)
    logger.info(stream)
    ...
    solve_current = solve_lp(args,args.num_products,args.cardinality,GT_choice_model_list,products_price,initial_inventory)
    ...
    if args.only_test_Benchmark:
        stream = os.path.join('GT', GT_folder+'/Benchmark_INV'+str(args.ini_inv)+args.reuse_type)
        logger = setup_logger(name='Train', level=20, stream = stream)
        args.logger = logger
        logger.info("Load Factor {}".format(args.L/np.sum(initial_inventory)))
