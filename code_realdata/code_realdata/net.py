...
class A2C(nn.Module):
    def __init__(self, args):
        ...
        if not args.feature:
            ...
            self.cus_embedding = nn.Sequential(*[nn.Linear(args.num_cus_features, self.cus_embedding_dim),nn.ReLU()])
        #time
        ...
        if (args.num_products == 30 and args.h in [5]):
            args.share_lr=args.actor_lr=args.critic_lr=0.001
            args.lr_min=0.00005
        ...
        if args.h == 1 and args.num_products == 100:
            args.lr_decay_lambda = 0.995
        if args.h == 2 and args.num_products == 100:
            args.lr_decay_lambda = 0.9995
    ...
    def forward(self, x):
        ...
        if not self.MLP:
            ...
            score = self.actor(x)##########
            return score, value##########
    def roll_out(self, env, input_sequence, cus_sequence, i):
        ...
        for num_step in range(self.args.num_steps):
            ...
            cus_feature = torch.from_numpy(self.type_cus_feature[int(arriving_seg[0])])[torch.tensor(cus_sequence[:, i]).to(self.device)[0]].reshape(1,-1)
            
            if not self.feature:
                cus_type_embedding = self.cus_embedding(arriving_seg)
            if self.feature:
                cus_type_embedding = self.cus_embedding(cus_feature)
            ...
            _, reward = env.step(arriving_seg,cus_feature,assortment.numpy())
            #要输出的东西
            ...
            if num_step == self.args.num_steps-1 or env.all_finished():#这个roll out结束
                ...
                cus_feature = torch.from_numpy(self.type_cus_feature[int(arriving_seg[0])])[0].reshape(1,-1)
    ...
    def test_env(self,env,input_sequence,cus_sequence):
        ...
        while not (env.all_finished()):
            ...
            if self.MLP:
                ...
                assortment, p, ass_log_softmax = self._select_node_RNN(env,
                                x, mask.bool())  # Squeeze out steps dimension
            index_, reward = env.step(arriving_seg,cus_feature, assortment.numpy())
            R += reward
            if self.args.detail:
