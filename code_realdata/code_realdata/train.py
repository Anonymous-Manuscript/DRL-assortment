...
from env import market_dynamic,market_dynamic_feature
from other_agents import OA_agent,myopic_agent,E_IB_agent,sub_t_agent,DP_Greedy_agent,DP_Rollout_agent
import torch
from numpy import *
...
def train(best,i,args,simulator_list,mnl_etas_list,products_price,train_sequences,train_cus_feature_index_list,logger):
    ...
    if args.CM == 'GT':
        env = market_dynamic_feature(args, simulator_list, products_price)
    else:
        env = market_dynamic(args, simulator_list, products_price)

    total_val_reward = val(args,env,seller,initial_inventory,train_sequences,train_cus_feature_index_list)
    ...
    for epoch in range(args.epoch_num):
        ...
        for seq_index in seq_indexlist:
            ...
            input_sequence = np.array([input_sequence]).astype(int)
            cus_sequence = np.array([train_cus_feature_index_list[seq_index]]).astype(int)
            env.reset(initial_inventory, T)
            if args.DQN:
                total_reward = train_batch_DQN(args, seller, lr_scheduler, optimizer, env, input_sequence, cus_sequence)
            else:
                if not args.GR_Train:
                    seller.set_decode_type('sampling')
                total_reward = train_batch(args, seller, lr_scheduler, optimizer, env, input_sequence, cus_sequence)
        if not args.DQN:
            seller.set_decode_type('greedy')
        total_val_reward = val(args,env,seller,initial_inventory,train_sequences,train_cus_feature_index_list)
...
def train_batch(args,
    model, lr_scheduler, optimizer, env, input_sequence, cus_sequence):
    # Evaluate model, get costs and log probabilities
    i = 0
    total_reward = 0
    while not (env.all_finished()):
        log_probs, values, rewards, mean_entropy, m_dones, i, next_value = \
            model.roll_out(env, input_sequence, cus_sequence, i)
...
def train_batch_DQN(args,
    model, lr_scheduler, optimizer, env, input_sequence, cus_sequence,train_=True):
    #给一个batch的input_sequence，训练模型
    total_reward = 0
    i = 0
    arriving_seg = input_sequence[:, i]
    cus_feature = torch.from_numpy(model.type_cus_feature[int(arriving_seg[0])])[torch.tensor(cus_sequence[:, i]).to(model.device)[0]].reshape(1,-1)
    ...
    if not args.feature:
        cus_type_embedding = model.cus_embedding(arriving_seg)
    if args.feature:
        cus_type_embedding = model.cus_embedding(cus_feature)
    ...
    if args.feature:
        state = torch.cat((s,cus_feature.reshape(1,-1)), dim=1)
        state = torch.cat((state,torch.from_numpy(arriving_seg).reshape(1,-1)), dim=1)
    ...
    while not (env.all_finished()):
        ...
        ###############################
        _, reward = env.step(arriving_seg, cus_feature, assortment)
        total_reward += reward[0]
        
        i +=1
        next_arriving_seg = input_sequence[:, i]
        cus_feature = torch.from_numpy(model.type_cus_feature[int(next_arriving_seg[0])])[torch.tensor(cus_sequence[:, i]).to(model.device)[0]].reshape(1,-1)

        input = torch.tensor(env.inventory_level/env.initial_inventory)
        input = torch.cat((input, model.t_embedding(torch.tensor(env.arrivals).unsqueeze(0))), dim=1)
        if not args.feature:
            cus_type_embedding = model.cus_embedding(next_arriving_seg)
        if args.feature:
            cus_type_embedding = model.cus_embedding(cus_feature)
        input = torch.cat((input,cus_type_embedding), dim=1)
        ...
        if args.feature:
            next_state = torch.cat((s,cus_feature.reshape(1,-1)), dim=1)
            next_state = torch.cat((next_state,torch.from_numpy(next_arriving_seg).reshape(1,-1)), dim=1)
        
    
        if bool(train_):
            model.remember(state[0], assortment[0], reward[0], next_state[0], env.all_finished())
...
def val(args,env,seller,initial_inventory,train_sequences,train_cus_feature_index_list):
    ...
    for seq_index in seq_indexlist:
        ...
        #####################
        if args.DQN:
            cost = train_batch_DQN(args, seller, 0, 0, env, input_sequence, cus_sequence,train_=False)
        else:
            cost,test_value = seller.test_env(env, input_sequence, cus_sequence)#######################
        #####################
...
def test(test_sequences,test_cus_feature_index_list,GT_choice_model,mnl_list,products_price,args,logger,only_test_benchmark=False,only_test_DRL=False):
    initial_inventory = np.array([args.ini_inv] * args.num_products)
    for seed_ in range(args.seed_range):
        
        env_OA = market_dynamic_feature(args,GT_choice_model,products_price)
        env_myopic = market_dynamic_feature(args,GT_choice_model,products_price)
        env_EIB = market_dynamic_feature(args,GT_choice_model,products_price)
        env_sub = market_dynamic_feature(args,GT_choice_model,products_price)
        env_DP_Greedy = market_dynamic_feature(args,GT_choice_model,products_price)
        env_DP_Rollout = market_dynamic_feature(args,GT_choice_model,products_price)
        ...
        env = market_dynamic_feature(args,GT_choice_model,products_price)
        if args.cus_onehot:
            seller = A2C_CusOnehot(args).to(args.device)
        else:
            seller = A2C(args).to(args.device)
        ...
        for seq_index in seq_indexlist:
            ...
            T = len(input_sequence)
            input_sequence = np.array([input_sequence]).astype(int)
            cus_sequence = np.array([test_cus_feature_index_list[seq_index]]).astype(int)
            
            if only_test_benchmark:
                other_agents(args,OA_seller, myopic_seller, E_IB_seller,sub_t_seller,DP_Greedy_seller,DP_Rollout_seller,initial_inventory, T, products_price, input_sequence, cus_sequence)
                cost = np.zeros((args.batch_size, 1))
            else:
                env.reset(initial_inventory, T)
                if args.detail:
                    cost,test_value,change_of_inv,change_of_R,offer_matrix,purchase_matrix = seller.test_env(env, input_sequence, cus_sequence)
...
def test_DQN(test_sequences,test_cus_feature_index_list,GT_choice_model,mnl_etas_list,initial_inventory,products_price,args,logger,load):
    for seed_ in range(args.seed_range):
        env = market_dynamic_feature(args,GT_choice_model,products_price)
...
def other_agents(args,OA_seller,myopic_seller,E_IB_seller,sub_t_seller,DP_Greedy_seller,DP_Rollout_seller,
                 initial_inventory,T,products_price,input_sequence, cus_sequence):
    ...
    for t in range(T-1):
        ...
        arriving_cus = torch.from_numpy(args.type_cus_feature[int(arriving_seg[0][0])][cus_sequence[0,t]]).reshape(1,-1)
        OA_seller.step(arriving_seg,arriving_cus)
        myopic_seller.step(arriving_seg,arriving_cus)
        E_IB_seller.step(arriving_seg,arriving_cus)
        sub_t_seller.step(arriving_seg,arriving_cus)
        DP_Greedy_seller.step(arriving_seg,arriving_cus)
        DP_Rollout_seller.step(arriving_seg,arriving_cus)
