...
class DQN:
    def __init__(self,args,mnl_list,seed):
        ...
        self.num_cus_features = args.num_cus_features
    ...
    def update(self,optimizer,scheduler):#based on self.feature==True
        ...
        transfered_state_batch = torch.cat((transfered_state_batch, self.cus_embedding(state_batch[:,(self.num_products+1):-1].to(torch.float64))),dim=1)
        ...
        transfered_nextState_batch = torch.cat((transfered_nextState_batch, self.cus_embedding(nextState_batch[:,(self.num_products+1):-1].to(torch.float64))),dim=1)
