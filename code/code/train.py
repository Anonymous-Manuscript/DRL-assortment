from net import A2C,A2C_CusOnehot
from net_PPO import PPO
from net_PG import PG
...
def train_PPO(best,i,args,simulator_list,mnl_etas_list,products_price,train_sequences,logger):
    initial_inventory = np.array([args.ini_inv] * args.num_products)
    torch.manual_seed(args.net_seed + i)
    random.seed(args.net_seed + i)

    seller = PPO(args).to(args.device)
    seller.set_decode_type('greedy')
    lr_scheduler, optimizer = initialize(args, seller)
    ...
    for epoch in range(args.epoch_num):
        ...
        for seq_index in seq_indexlist:
            ...
            seller.train_batch(args, lr_scheduler, optimizer, env, input_sequence)
...
def train_PG(best,i,args,simulator_list,mnl_etas_list,products_price,train_sequences,logger):
    initial_inventory = np.array([args.ini_inv] * args.num_products)
    torch.manual_seed(args.net_seed + i)
    random.seed(args.net_seed + i)

    seller = PG(args).to(args.device)
    seller.set_decode_type('greedy')
    lr_scheduler, optimizer = initialize_PG(args, seller)
...
def test(test_sequences,GT_choice_model_list,mnl_list,products_price,args,logger,only_test_benchmark=False,only_test_DRL=False):
    ...
    for seed_ in range(args.seed_range):
        torch.manual_seed(args.seed+seed_)
        random.seed(args.seed+seed_)
        ...
        for seq_index in seq_indexlist:
            ...
            if only_test_benchmark:
                ...
                #print(myopic_seller.market.inventory_level)
                #breakpoint()
        ...
        if args.detail:
            ...
            np.save(folder + 'change/offer_matrix'+str(seed_)+'.npy',np.array(offer_matrix_sum))
            np.save(folder + 'change/purchase_matrix'+str(seed_)+'.npy',np.array(purchase_matrix_sum))
            other_agents_detail(args, seed_, OA_seller, myopic_seller, E_IB_seller,sub_t_seller,DP_Greedy_seller,DP_Rollout_seller,
                        initial_inventory, products_price, seq_indexlist, test_sequences)
...
def other_agents_detail(args, seed_, OA_seller, myopic_seller, E_IB_seller,sub_t_seller,DP_Greedy_seller,DP_Rollout_seller,
                        initial_inventory, products_price, seq_indexlist, test_sequences):
    change_of_inv_list = {}
    change_of_R_list = {}
    offer_matrix_sum = {}
    purchase_matrix_sum = {}
    for agent in ['OA_seller', 'myopic_seller', 'E_IB_seller', 'DP_Greedy_seller']:
        change_of_inv_list[agent] = []
        change_of_R_list[agent] = []
        offer_matrix_sum[agent] = np.zeros((args.num_products+1,110))
        purchase_matrix_sum[agent] = np.zeros((args.num_products+1,110))
    for seq_index in seq_indexlist:
        input_sequence = test_sequences[seq_index]
        T = len(input_sequence)
        input_sequence = np.array([input_sequence])
        ...
        for agent,class_ in zip(['OA_seller', 'myopic_seller', 'E_IB_seller', 'DP_Greedy_seller'],[OA_seller, myopic_seller, E_IB_seller, DP_Greedy_seller]):
            change_of_inv_list[agent].append(np.pad(np.array(class_.change_of_inv_list) ,((0,110-T),(0,0)),'edge' ))
            change_of_R_list[agent].append(np.pad(np.array(class_.change_of_R_list) ,(0,110-T),'edge' ))
            offer_matrix_sum[agent] += class_.offer_matrix
            purchase_matrix_sum[agent] += class_.purchase_matrix
            
    folder = 'log/' + args.folder+'/DRLTrainLog'+ args.saved_time_stream        
    for agent in ['OA_seller', 'myopic_seller', 'E_IB_seller', 'DP_Greedy_seller']:
        
        np.save(folder + 'change/'+agent+'_change_of_inv_list'+str(seed_)+'.npy',np.array(change_of_inv_list[agent]))
        np.save(folder + 'change/'+agent+'_change_of_R_list'+str(seed_)+'.npy',np.array(change_of_R_list[agent]))
        offer_matrix_sum[agent] = offer_matrix_sum[agent]/len(seq_indexlist)
        purchase_matrix_sum[agent] = purchase_matrix_sum[agent]/len(seq_indexlist)
        np.save(folder + 'change/'+agent+'_offer_matrix'+str(seed_)+'.npy',np.array(offer_matrix_sum[agent]))
        np.save(folder + 'change/'+agent+'_purchase_matrix'+str(seed_)+'.npy',np.array(purchase_matrix_sum[agent]))
