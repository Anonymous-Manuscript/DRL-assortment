...
class A2C(nn.Module):
    def __init__(self, args):
        ...
        if args.T == 100 or (args.num_products == 20 and args.MLP == True and args.cardinality == 4) or (args.num_cus_types == 20 and args.cus_embedding_dim == 10) or (args.GT == 'lcmnl' and args.CM in ['net']) or (args.GT == 'lcmnl' and args.CM in ['lcmnl']):
            args.w = [32,64]
        if (args.num_products == 20 and args.h == 1):
            args.lr_decay_lambda = 0.99
        if args.T == 50:
            args.step = 10
        if (args.GT == 'rl' and args.CM in ['GT']) or (args.GT == 'gsp' and args.CM in ['rcs']): 
            args.w = [32,32]
        share = []
...
class A2C_CusOnehot(nn.Module):
    def __init__(self, args):
        super(A2C_CusOnehot, self).__init__()
        ...
        share = []
        share.append(nn.Linear(args.num_products+self.num_cus_types+self.t_embedding_dim, args.w[0]))#
        share.append(nn.ReLU())
        for h in range(args.h-1):
            share.append(nn.Linear(args.w[h], args.w[h+1]))
            share.append(nn.ReLU())
    ...
    def roll_out(self, env, input_sequence, i):
        ...
        for num_step in range(self.args.num_steps):
            ...
            s = env.inventory_level/env.initial_inventory
            s = torch.tensor(np.hstack((s,cus_type))).to(self.device)
            s = torch.cat((s,self.t_embedding(torch.tensor(env.arrivals).to(self.device).unsqueeze(0))), dim=1)
            # 关键语句
            x, value = self.forward(s)
            #########
            ...
            if num_step == self.args.num_steps-1 or env.all_finished():#这个roll out结束
                if env.all_finished():
                    next_value = 0#done之后这个值是没用的
                    break
                arriving_seg = np.random.randint(1, self.num_cus_types, (1,))
                cus_type = self.cus_type[arriving_seg]
                next_state = env.inventory_level/env.initial_inventory
                next_state = torch.tensor(np.hstack((next_state,cus_type))).to(self.device)
                next_state = torch.cat((next_state,self.t_embedding(torch.tensor(env.arrivals).to(self.device).unsqueeze(0))), dim=1)
                _, next_value = self.forward(next_state)
                break
        # Collected lists, return Tensor
